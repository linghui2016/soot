...
public class SparkTransformer extends SceneTransformer {
  ...
  protected void internalTransform(String phaseName, Map<String, String> options) {
    ...
    if (opts.cs_demand()) {
      ...
      PointsToAnalysis onDemandAnalysis = DemandCSPointsTo.makeWithBudget(opts.traversal(),
          opts.passes(), opts.lazy_pts());
      Date endOndemand = new Date();
      reportTime("Initialized on-demand refinement-based context-sensitive analysis", startOnDemand,
          endOndemand);
  ...
  protected static void reportTime(String desc, Date start, Date end) {
    ...
    // G.v().out.println("[Spark] " + desc + " in " + time / 1000 + "." + (time / 100) % 10 + "
    // seconds.");
  }
